...
export const Edges = () => {
  const edges = React.useContext(DataContext).Edges || new Map()

  const data = useHardMemo(() => _.map([...edges], ([[Name], e]) => ({ Name, Source: e.Source, ...e })))
        ...
        <tbody {...getTableBodyProps()}>
          {rows.map(row => {
            ...
            return (
              <tr {...row.getRowProps()}>
                {row.cells.map(cell => (
                  <td {...cell.getCellProps()} style={{ background: cell.column.isSorted ? '#ccddff44' : undefined }}>
